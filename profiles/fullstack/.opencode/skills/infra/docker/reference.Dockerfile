...
ARG RRA_API_SLUG="rest.rt-analyzer"
...
ARG RRA_HOME_DIR="/app"
ARG RRA_API_DIR="${RRA_HOME_DIR}/${RRA_API_SLUG}"
ARG RRA_API_DATA_DIR="/var/lib/${RRA_API_SLUG}"
ARG RRA_API_LOGS_DIR="/var/log/${RRA_API_SLUG}"
ARG RRA_API_TMP_DIR="/tmp/${RRA_API_SLUG}"
ARG RRA_API_PORT=8000
## IMPORTANT!: Get hashed password from build-arg!
## echo "RRA_USER_PASSWORD123" | openssl passwd -5 -stdin
...
ENV RRA_HOME_DIR="${RRA_HOME_DIR}" \
    RRA_API_DIR="${RRA_API_DIR}" \
    RRA_API_DATA_DIR="${RRA_API_DATA_DIR}" \
    RRA_API_LOGS_DIR="${RRA_API_LOGS_DIR}" \
    RRA_API_TMP_DIR="${RRA_API_TMP_DIR}" \
    RRA_API_PORT=${RRA_API_PORT} \
    UID=${UID} \
    GID=${GID} \
    USER=${USER} \
    GROUP=${GROUP} \
...
RUN rm -rfv /var/lib/apt/lists/* /var/cache/apt/archives/* /tmp/* /root/.cache/* && \
    ...
    mkdir -pv "${RRA_API_DIR}" "${RRA_API_DATA_DIR}" "${RRA_API_LOGS_DIR}" "${RRA_API_TMP_DIR}" && \
    chown -Rc "${USER}:${GROUP}" "${RRA_HOME_DIR}" "${RRA_API_DATA_DIR}" "${RRA_API_LOGS_DIR}" "${RRA_API_TMP_DIR}" && \
    find "${RRA_API_DIR}" "${RRA_API_DATA_DIR}" -type d -exec chmod -c 770 {} + && \
    find "${RRA_API_DIR}" "${RRA_API_DATA_DIR}" -type d -exec chmod -c ug+s {} + && \
    find "${RRA_API_LOGS_DIR}" "${RRA_API_TMP_DIR}" -type d -exec chmod -c 775 {} + && \
    find "${RRA_API_LOGS_DIR}" "${RRA_API_TMP_DIR}" -type d -exec chmod -c +s {} +
...
WORKDIR "${RRA_API_DIR}"
COPY --chown=${UID}:${GID} ./src ${RRA_API_DIR}
COPY --chown=${UID}:${GID} --chmod=770 ./scripts/docker/*.sh /usr/local/bin/

# VOLUME ["${RRA_API_DATA_DIR}"]
# EXPOSE ${RRA_API_PORT}

USER ${UID}:${GID}
# HEALTHCHECK --start-period=30s --start-interval=1s --interval=5m --timeout=5s --retries=3 \
#     CMD curl -f http://localhost:${RRA_API_PORT}/api/v${RRA_API_VERSION:-1}/ping || exit 1

ENTRYPOINT ["docker-entrypoint.sh"]
# CMD ["-b", "uvicorn main:app --host=0.0.0.0 --port=${RRA_API_PORT:-8000} --no-access-log --no-server-header --proxy-headers --forwarded-allow-ips='*'"]
