...
- Missing `# syntax=docker/dockerfile:1` directive — BuildKit cache/bind mounts unavailable.
- Copy-then-delete of dependency manifests instead of BuildKit mounts (`--mount=type=cache`, `--mount=type=bind`).
- `apt-get install` without `--no-install-recommends`, or apt lists/caches not cleaned in the same `RUN` layer.
- Startup logic inlined in `CMD` instead of a `docker-entrypoint.sh`; an entrypoint that does not `exec` the process or run it under `tini` (the process never becomes PID 1, so signals are dropped).
- Root not dropped: no final `USER ${UID}:${GID}` and no step-down (`gosu`/`su-exec`) inside the entrypoint.
- `COPY` followed by a separate `chown`/`chmod` layer instead of `COPY --chown=`/`--chmod=`.
...
- Flag a root main process (no final `USER` and no entrypoint step-down) and missing `docker-entrypoint.sh` / `exec` / `tini` as WARNING.
- Flag a missing `# syntax` directive and dependency installs that don't use BuildKit cache/bind mounts as SUGGESTION.
